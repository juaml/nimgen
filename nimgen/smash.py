...
def export_significance_genes(reference_data, smashed_data):
    ...
    empirical_pval = empirical_pval(stat, stat0)
    df = pd.DataFrame({'genes': reference_data.index, 'pval': empirical_pval}).set_index('genes')
    reject, corrected, *_ = multipletests(df['pval'], alpha=0.05, method='fdr_bh', is_sorted=False, returnsorted=False)
    df['fdr'] = corrected
